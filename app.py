...
def startup_sync_from_gcs():
    ...
    GCS is the source of truth — local defaults are never pushed to GCS.
    """
...
def fetch_all_market_indices(target_date=None):
    """Fetch all global market indices from Yahoo Finance."""
    results = []
    for idx in MARKET_INDICES:
        ...
        results.append({
            ...
            "date":  data.get("date", str(datetime.now(KST).date())),
...
def webhook_run_analysis():
    ...
    # 실행 전 GCS에서 최신 설정 동기화 (Cloud Run 콜드스타트 여부와 무관하게 항상 최신값 사용)
    startup_sync_from_gcs()
